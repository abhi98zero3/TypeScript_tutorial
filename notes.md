...
4. **In functions**, parameters should be inferred with types since 'any' is assigned to them by default and this will cause issues when it comes to getting desired results from that function.
    ...
    We can provide the parameters a default value so that they can be provided optionally when that function is called, preventing error to arise in case no value is provided for such args.
...
Generic Classes - They lets us create reusable components that work with a variety of data types while preserving type safety.
    ...
    ```
14. **Type narrowing in TS** - It is the proceess of refining the types of a variable from a broader union type to a more specific one based on runtime checks, allowing typescript to provide better type safety and autocomplete support within specific code blocks.
Syntax : 
```
...
```
It can be done by using conditionals on 'typeof' which is used on primitive types such as number, string, boolean, etc. as well as by using 'instanceof' which is used for checking if the variable is an instance of a specific class (in case it is constructed using new keyword)

-> **Type predicates** - In TS, Type predicates are used for type narrowing of a variable within a conditional block.
It is a return type of a function that tells TS that if a function returns true, the input is of a specific type.
Syntax : 
```
...
```
Here, if the function returns `true`, the variable pet will be typecasted as `Fish`

-> **Discriminated Union** - These are patterns where each type in a union has a common literal property (like '`kind`') that helps TS distinguish between them. This allows exhaustiveness checking using switch statements so that if a case is missed, it can be catched at compile time
Example : 
```
interface Circle  {
    kind : "circle",
...
The `never` type is used in the default case to ensure all possible variants have been handled. If a new variant is added and not handled, TypeScript will throw an error, making your code more robust.
